...
function prototype is ``f (int x)``.  Likewise `(inout) (nullable)` for an `int` parameter
...
::
    ...
    *     The callee can modify the data of the parameter in a way that
    *     is visible to the caller, that is the (in) variable can have
    *     output semantics.  f1_in (NULL) is valid.
    */
    void f1_in (char *x);
...
::
    ...
    *     parameter.  The callee can modify the data of the parameter in
    *     a way that is visible to the caller, that is the (in) variable
    *     can have output semantics.  f1_in (NULL) is invalid.
    */
    void f2_in (char *x);
